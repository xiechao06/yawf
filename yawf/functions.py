...
def register_policy(policy_cls):
    ...
    :raise yawf.exceptions.PolicyAlreadyRegistered: if policy already
    registered
...
def register_policies(*policy_cls_lst):
    '''
    register a list of polices
    '''

    for policy_cls in policy_cls_lst:
        yawf.WorkFlowEngine.instance.register_policy(policy_cls)

def clear_polices():
    yawf.WorkFlowEngine.instance.clear_polices()


def new_work_flow(tag, node_creator, tag_creator=None, annotation=None,
                  token=None):
    """
    create a work flow
    ...
    """
    work_flow = do_commit(yawf.models.WorkFlow(tag=tag,
...
def token_bound(tag, token):
    ...
    return model.query.filter(
        and_(model.tag == tag,
             model.token == token,
             model.status == yawf.constants.WORK_FLOW_PROCESSING)).first()
